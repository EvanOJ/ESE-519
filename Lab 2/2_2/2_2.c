...
//Sets/defines all variables as unsigned integers
unsigned int diff, edge1, edge2,edgeCount,overflows;
uint32_t pulse_width;

ISR(TIMER1_COMPA_vect)																			//Interrupt service routine for timer 1 output compare interrupt
{
...
ISR(TIMER1_CAPT_vect)																			//Interrupt service routine for timer 1 input capture interrupt
{
	TIMSK1 |= (0<<OCIE1A);																		//Turn off output compare interrupts (is this needed if TIMSK1 |= (1<<ICIE) should change everything else to 0)
	DDRB |= (0<<PORTB0);																		//Set PB0 as input

	if(edgeCount %2 == 0)																		//If remainder ==0, this is a falling edge...
	{
		edge2 = ICR1;																			//Store time value of edge 2 to ICR1 register
		...
		pulse_width = (0.000005)*((long)overflows * 65536u +(long)diff);						//Calculate pulse width given the period and maximum number of clock ticks a 16-bit timer can have
...
int main(void)																					//Main loop
{
	...
	OCR1A = TCNT1 + 20;																			//Pull OCRA pin high quickly and initiate using 10us pinger (as per pinger manual suggestion)
	...
	TIMSK0 |= (1<<OCIE0A);																		//Set ISR ouptut compare vector
	...
	while (1)
	{
		...
		printf("pulse_width= %lu\n", (8000000/(64*freq_adj))-1);								//Print pulse width as an unsigned long s.t. pulse width is calculated using 64 prescaler and divided by 2 again to increase signal stability
